...
class PersonalUrls(generics.ListAPIView):
	...
	def get_queryset(self):
		return UrlPublish.objects.filter(usernameid_id= self.request.user.id)
...
class FollowList(generics.ListCreateAPIView):
	...
	filter_fields = ('followedid','followerid') 
	permission_classes = (
		permissions.IsAuthenticatedOrReadOnly,
	)
	...
	def perform_create(self,serializer):
		serializer.save(
			followerid = UserList.objects.get(username=self.request.user.username),
			followedid = UserList.objects.get(id=self.request.data['followedid'])
		)
...
class GroupMemberList(generics.ListCreateAPIView):
	...
	filter_backends = (DjangoFilterBackend,)
	filter_fields = ('groupid','memberid')
	permissions_classes = (
		permissions.IsAuthenticatedOrReadOnly,
	)
	def perform_create(self,serializer):
...
class ArticleCommentList(generics.ListCreateAPIView):
	...
	def perform_create(self, serializer):
		serializer.save(
			comment1=ArticlePublish.objects.get(id=int(self.request.GET.values()[0])),
			usernameid=UserList.objects.get(username=self.request.user.username)
		)
